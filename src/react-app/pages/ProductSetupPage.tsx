...
function PaymentMethodsPanel({ methods, onChange }: { methods: PaymentMethod[]; onChange: (value: PaymentMethod[]) => void }) {
	const t = useT(); const shell = useAppShell(); const [name, setName] = useState(""); const [kind, setKind] = useState<PaymentMethod["kind"]>("other"); const [pending, setPending] = useState(false); const [failed, setFailed] = useState(false);
	async function submit(event: FormEvent) { event.preventDefault(); if (!name.trim()) return; setPending(true); setFailed(false); try { const method = await productSetupApi.createPaymentMethod(shell.organizationId, { name, kind }); onChange([...methods, method]); setName(""); } catch { setFailed(true); } finally { setPending(false); } }
	...
	return <section className="space-y-4 rounded-xl border p-4"><div><h2 className="text-lg font-semibold">{t("Payment methods")}</h2><p className="text-sm text-muted-foreground">{t("Cash is ready by default. Add simple labels such as Nequi or Bancolombia.")}</p></div><ul className="grid gap-2">{methods.map((method) => <li key={method.id} className="flex min-h-12 items-center justify-between gap-3 rounded-lg bg-muted/50 p-3"><div><strong>{method.systemKey === "cash" ? t("Cash") : method.name}</strong><span className="ml-2 text-sm text-muted-foreground">{t(method.isActive ? "Active" : "Inactive")}</span></div><Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => void toggle(method)}>{t(method.isActive ? "Deactivate" : "Activate")}</Button></li>)}</ul>
		<form onSubmit={submit} className="grid gap-3 border-t pt-4"><h3 className="font-medium">{t("Add payment method")}</h3><div className="grid gap-2"><Label htmlFor="payment-method-name">{t("Method name")}</Label><Input id="payment-method-name" value={name} maxLength={80} placeholder={t("Example: Nequi")} required onChange={(event) => setName(event.target.value)} /></div><div className="grid gap-2"><Label htmlFor="payment-method-kind">{t("Method type")}</Label><select id="payment-method-kind" className="h-10 rounded-md border bg-background px-3" value={kind} onChange={(event) => setKind(event.target.value as PaymentMethod["kind"])}><option value="wallet">{t("Digital wallet")}</option><option value="bank">{t("Bank")}</option><option value="card">{t("Card")}</option><option value="cash">{t("Cash")}</option><option value="other">{t("Other")}</option></select></div>{failed ? <p role="alert" className="text-sm">{t("We could not save the payment method.")}</p> : null}<Button disabled={pending || !name.trim()}>{t(pending ? "Saving…" : "Add payment method")}</Button></form>
	</section>;
}
